...
class SettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        // 🎨 Thème entreprise : JSON limité aux couleurs reconnues.
        $rules['theme_config'] = ['nullable', 'string', 'max:4000', function ($attribute, $value, $fail) {
            if (trim((string) $value) === '') return;
            $decoded = json_decode((string) $value, true);
            if (!is_array($decoded)) {
                $fail('La configuration du thème doit être un JSON valide.');
                return;
            }
            $custom = is_array($decoded['custom'] ?? null) ? $decoded['custom'] : $decoded;
            foreach (['primary', 'primaryDark', 'accent', 'success', 'danger', 'warning', 'info'] as $key) {
                if (array_key_exists($key, $custom) && !preg_match('/^#[0-9A-Fa-f]{6}$/', (string) $custom[$key])) {
                    $fail("La couleur {$key} doit être au format hexadécimal.");
                    return;
                }
            }
        }];
