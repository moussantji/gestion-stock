...
class ProductController extends Controller
{
    ...
    /**
     * POST /api/products/bulk-price — Mise à jour des prix en lot.
     * Body : { products: [{ id: int, sale_price: float }] }
     */
    public function bulkPrice(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $now, $userId, &$updated) {
            foreach ($request->input('products') as $item) {
                $product = Product::find($item['id']);
                if (!$product) continue;

                $oldPrice = $product->sale_price;
                $newPrice = (float) $item['sale_price'];
    ...
    /**
     * GET /api/products/{product}/price-history — Historique des changements de prix.
     */
