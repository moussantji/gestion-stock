...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $currentMoves = ShopScope::apply(StockMovement::query(), $request)
            ->whereBetween('created_at', [now()->subDays(6)->startOfDay(), now()->endOfDay()])
            ->sum('quantity');
        $previousMoves = ShopScope::apply(StockMovement::query(), $request)
            ->whereBetween('created_at', [now()->subDays(13)->startOfDay(), now()->subDays(7)->endOfDay()])
            ->sum('quantity');
        $movementTrend = $previousMoves > 0
            ? (int) round((($currentMoves - $previousMoves) / $previousMoves) * 100)
            : ($currentMoves > 0 ? 100 : 0);
