...
const RECEIPT_ICON_MAP = {
  '📄': 'receipt', '🖨': 'printer', '📱': 'language', '🧾': 'sale', '🔵': 'printer', '↩️': 'movements', '⛔': 'close',
};
...
export default function ReceiptActionsSheet({ receipt, onClose, navigation, onChanged }) {
  ...
  const styles = useThemedStyles(c => ({
    ...
    cancelText: { color: c.muted, fontWeight: '700', fontSize: 13.5 },
    ...
    cashLabel: { color: c.text, fontSize: 13, fontWeight: '700' },
    cashHint: { color: c.muted, fontSize: 11, marginTop: 2 },
    refundedBadge: {
    ...
    refundedText: { color: c.danger, fontWeight: '800', fontSize: 13 },
    partialHint: { fontSize: 11.5, color: c.muted, textAlign: 'center', marginBottom: 10 },
    lineRow: {
      flexDirection: 'row',
      alignItems: 'center',
      backgroundColor: c.card,
    ...
    lineName: { fontSize: 13.5, fontWeight: '700', color: c.text },
    lineMax: { fontSize: 11, color: c.muted, marginTop: 2 },
    ...
    return (
      ...
      >
        {loading ? (
          ...
          <Icon name={RECEIPT_ICON_MAP[icon] ?? icon} size={19} color={danger ? colors.danger : warning ? colors.warning : accent ? colors.accent : colors.primary} />
        )}
