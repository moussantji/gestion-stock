...
const TABS = [
  { key: 'overview', label: 'Résumé', icon: 'stats' },
  { key: 'orders', label: 'Commandes', icon: 'sale' },
  { key: 'licenses', label: 'Abonnements', icon: 'profile' },
];

const ORDER_BADGE = {
  pending: { label: 'En attente', icon: 'clock', color: colors.warning },
  paid: { label: 'Payée', icon: 'success', color: colors.success },
  cancelled: { label: 'Annulée', color: colors.muted },
};

const LICENSE_BADGE = {
  active: { label: 'Active', icon: 'success', color: colors.success },
  expired: { label: 'Expirée', icon: 'clock', color: colors.muted },
  revoked: { label: 'Révoquée', icon: 'close', color: colors.danger },
};
...
export default function AdminScreen() {
  ...
  const styles = useThemedStyles(c => ({
    ...
    tabText: { fontSize: 12.5, fontWeight: '800', color: c.muted },
    grid: { flexDirection: 'row' },
    sectionTitle: { fontSize: 16, fontWeight: '800', color: c.text, marginTop: 22, marginBottom: 10 },
    card: {
      backgroundColor: c.card,
      borderRadius: 14,
      padding: 14,
    ...
    ref: { fontSize: 13, fontWeight: '800', color: c.text, fontFamily: 'monospace' },
    badge: { fontSize: 12, fontWeight: '800' },
    client: { fontSize: 13.5, fontWeight: '700', color: c.text, marginTop: 8 },
    meta: { fontSize: 12.5, color: c.muted, marginTop: 3 },
    meta2: { fontSize: 11.5, color: c.muted, marginTop: 2, fontFamily: 'monospace' },
    subChip: { fontSize: 12.5, color: c.accent, fontWeight: '700', marginTop: 6 },
    planBig: { fontSize: 14.5, fontWeight: '800', color: c.text },
    ...
    shopName: { fontSize: 15, fontWeight: '800', color: c.text },
    logoHint: { fontSize: 10.5, color: c.muted, marginTop: 4 },
    logoBtn: {
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            {shop?.logo_url ? (
              ...
              <View style={[styles.logoPreview, styles.logoPlaceholder]}>
                <Icon name="shops" size={24} color={colors.muted} />
              </View>
            )}
      ...
      <View key={o.id} style={styles.card}>
        <View style={styles.cardHeader}>
          ...
          <View style={{flexDirection: 'row', alignItems: 'center', gap: 3}}>
                {badge.icon ? <Icon name={badge.icon} size={12} color={badge.color} /> : null}
        ...
        {o.license ? (
          <View style={{flexDirection: 'row', alignItems: 'center', gap: 3}}>
            <Icon name="profile" size={13} color={colors.accent} />
        ...
        {!compact || o.status === 'pending' ? (
          <View style={styles.cardActions}>
            {o.status === 'pending' ? (
              ...
              >
                {busyId === o.id ? <ActivityIndicator size="small" color={colors.success} /> : <View style={{flexDirection: 'row', alignItems: 'center', gap: 3}}>
                    <Icon name="success" size={14} color={colors.success} />
              ...
              <View style={styles.cardActions}>
                ...
                >
                  {busyId === l.id ? (
                    ...
                    <View style={{flexDirection: 'row', alignItems: 'center', gap: 3}}>
                    <Icon name="password" size={14} color={colors.primary} />
                ...
                >
                  <View style={{flexDirection: 'row', alignItems: 'center', gap: 3}}>
                    <Icon name={l.status === 'active' ? 'close' : 'success'} size={14} color={l.status === 'active' ? colors.danger : colors.success} />
        ...
        {TABS.map((t) => (
          ...
          >
            <View style={{flexDirection: 'row', alignItems: 'center', gap: 4, justifyContent: 'center'}}>
                <Icon name={t.icon} size={14} color={tab === t.key ? '#fff' : colors.muted} />
